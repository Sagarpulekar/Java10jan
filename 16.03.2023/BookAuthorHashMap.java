...
public class BookAuthorHashMap {
	...
	{
		HashMap<String, ArrayList<String>> authmap= new HashMap<>();
		...
		{
			Book b= itr.next();
			if(!authmap.containsKey(b.getAuthor().getAname()))
			{
			...
			{
				booklist= authmap.get(b.getAuthor().getAname());
				//booklist.add(b.getBname());
				//authmap.put(b.getAuthor(), booklist);
			}
			booklist.add(b.getBname());
			authmap.put(b.getAuthor().getAname(), booklist);
